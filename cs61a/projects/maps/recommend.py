...
def find_closest(location, centroids):
    ...
    [2.0, 3.0]
    """
    #use key to apply distance function to location and each (c)entroid in the list of centroids
...
def group_by_centroid(restaurants, centroids):
    ...
    #x in pair should be closest centeroid, y shoiuld be restaurant
...
def rate_all(user, restaurants, feature_fns):
    ...
    for restaurant in restaurants:
        ...
        r_name = restaurant_name(restaurant)
        if r_name in reviewed_names: 
            #print("yes")
            rest[r_name] = int(user_rating(user, r_name)) #restaurant name is key, user value int
        else:
            rest[r_name]=predictor(restaurant)  #restaurant name is key, predicted value decimal
    return rest
    # END Question 9
