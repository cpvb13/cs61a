...
class Place(object):
    ...
    def __init__(self, name, exit=None):
        ...
        # BEGIN Problem 2
        if self.exit: # if the Place has an exit
            exit.entrance = self # exit's entrance is set to that Place aka "self"
        # END Problem 2
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                # BEGIN Problem 9
                #if self.ant exists (there's already an ant in place)
                #check if it is has an empty container
                if self.ant.can_contain(insect):
                    self.ant.contain_ant(insect)

                elif insect.can_contain(self.ant): #if the insect you want to add is the container, give it a contained ant
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            # BEGIN Problem 13
            if isinstance(insect,QueenAnt) and insect.queen_or_nah:
                return
            # END Problem 13
...
class Insect(object):
    ...
    def __init__(self, armor, place=None):
        ...
        #Place.add_insect(self.place,self)
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        place_throws = self.place
        places_away =0

        while place_throws != hive:
            if self.min_range <= places_away <= self.max_range:
                bees = place_throws.bees
                if bees: #if there are bees in the list of this place...
                    return random_or_none(bees)
            place_throws = place_throws.entrance # will get the place object in front
            places_away += 1
        # END Problem 3 and 4
...
class HungryAnt(Ant):
    ...
    def eat_bee(self, bee):
        ...
        rand_bee = random_or_none(bee)
        rand_bee.reduce_armor(rand_bee.armor)
        
        #print(bee[rand_bee].armor)
        # END Problem 6

    def action(self, colony):
        # BEGIN Problem 6
        if self.turns: #truthy val greater than zero
            self.turns -= 1
        else: #equal zero
            if self.place.bees: #if there are bees in the place
                self.turns = self.time_to_digest
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        # BEGIN Problem 7
        for bee in self.place.bees[:]: #make copy of list
            bee.reduce_armor(self.damage)
...
class BodyguardAnt(Ant):
    ...
    implemented = True   # Change to True to view in the GUI
    def __init__(self, armor=2,place = None):
        """Create an Ant with an ARMOR quantity."""
        Insect.__init__(self, armor,place)
        self.contained_ant = None #Instance attr because its diff for each guard
    # END Problem 9
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def __init__(self, armor=1 , place = None):
        # BEGIN Problem 13
        ScubaThrower.__init__(self,armor,place)
        if QueenAnt.queen_or_nah:
            self.queen_or_nah = True
            QueenAnt.queen_or_nah = False
        else:
            self.queen_or_nah = False

        # END Problem 13
    def double_dooey(dis_ant):
        #does an ant exist in the place? make sure its not a queen... make sure its not buff...
        if dis_ant and dis_ant.name is not 'Queen' and dis_ant not in QueenAnt.buff_boiz:
            dis_ant.damage *= 2
            QueenAnt.buff_boiz.append(dis_ant)
        #same logic as above, but when instance attr of is_container is true
        if dis_ant and dis_ant.is_container and dis_ant.contained_ant: #is it holding something?
            QueenAnt.double_dooey(dis_ant.contained_ant) #call same function on it
    ...
    def action(self, colony):
        ...
        """
        # BEGIN Problem 13
        if self.queen_or_nah:
            ScubaThrower.action(self,colony)
            check_place = self.place.exit
            while check_place:
                QueenAnt.double_dooey(check_place.ant)
...
def make_slow(action, bee):
    ...
    def slow_action(colony):
        """Return either action or none depending on colony time being even/odd.
        """
        #print('c',colony.time)
        if colony.time % 2 == 0:
    ...
    #this did not work but i have a question about it
    #'or' returns first true, so when mod is not zero, it returns some int, else it does action
    #this did not work because even if action(colony) was the Truthy value, i think it gets called twice (once while evaluating and once when returning it??)
    #return lambda colony: colony.time % 2 == 0 or action(colony)
    # END Problem EC
...
def make_scare(action, bee):
    ...
    def scare_action(colony):
        nonlocal scare_action
        if bee.is_spooked is False:
            bee.is_spooked = 2
        return action(colony)
    return scare_action
    # END Problem EC
...
def apply_effect(effect, bee, duration):
    ...
    # BEGIN Problem EC
    normal, effect = bee.action, effect(bee.action,bee)
    
    def which_action(colony):
        nonlocal duration
        if duration:
            duration -= 1
            return effect(colony)
        else:
            return normal(colony)
        
    
    bee.action = which_action
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, hive):
        ...
        place = self.place
        while place != hive:
            if place.ant and place.ant.name != 'Laser':
                get_wrecked[place.ant]= places_away
            for bee in place.bees:
                get_wrecked[bee] = places_away
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem OPTIONAL
        damage = self.damage - self.hurt_forreal*self.insects_shot
        hurted = damage - self.weak_temp*distance
        if hurted < 0:
            hurted = 0
        return hurted
        # END Problem OPTIONAL
