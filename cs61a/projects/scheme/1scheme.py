...
def scheme_eval(expr, env, tail=False): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        #print('there')
        # if _:
        #     first, second = expr.first, expr.second
        #     procedure = scheme_eval(first,env)
        #     check_procedure(procedure)
        #     arguments = second.map(lambda func: scheme_eval(func, env))
        #     return scheme_apply(procedure,arguments)
        ...
        # check_procedure(operator)
        return scheme_apply(operator,operands,env,tail)
        # END PROBLEM 5
...
def scheme_apply(procedure, args, env, tail = False):
    ...
    if isinstance(procedure, BuiltinProcedure):
        ...
        #print(procedure.body,'body&env',env)
...
class Frame:
    ...
    def lookup(self, symbol):
        ...
        elif self.parent and self.parent.lookup(symbol):
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('too much in list: {0}'.format(args))
        # END PROBLEM 4
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 6
        expression = scheme_eval(expressions.second.first,env)
        env.define(target,expression)
        return target
        # END PROBLEM 6
    elif isinstance(target, Pair) and scheme_symbolp(target.first):
        name = target.first
        # BEGIN PROBLEM 10
        expressions = Pair(target.second,expressions.second)
        lambdaex = do_lambda_form(expressions, env)
        env.define(name,lambdaex)
        return name
        # END PROBLEM 10
    else:
...
def do_and_form(expressions, env):
    ...
    while expressions is not nil:
        cond = scheme_eval(expressions.first,env)
        if scheme_falsep(cond):
            return cond
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 15
    formals,vals = nil, nil #need to pass in two lists for make_child_frame
    while bindings is not nil:
        first_bind = bindings.first
        second_bind = bindings.first.second
        check_form(first_bind,2,2)

        formals = Pair(first_bind.first,formals)
        vals = Pair(scheme_eval(second_bind.first,env),vals)
...
def optimize_tail_calls(original_scheme_eval):
    ...
    def optimized_eval(expr, env, tail=False):
        ...
        while isinstance(result,Thunk):
            ...
            expr = expr.second
        return result
        #repeatedly call original_scheme_eval until the result is a value, rather than a Thunk
        #A tail call occurs when a function calls another function as its last action of the current frame. 
        #In this case, the frame is no longer needed, and we can remove it from memory. 
        # In other words, if this is the last thing you are going to do in a function call, 
        #we can reuse the current frame instead of making a new frame.   
        #     print(first)
        #     print(second)
        #     print(tail)
        # END
